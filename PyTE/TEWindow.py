...
class TEWindow:
    ...
    def addOnlineObservationsLag1(self, source, dest, startTime=0, endTime=0):
        ...
        for r in list(range(startIndex, endIndex)):
            ...
            if self.k > 0:
                pastVal[0] -= self.maxShiftedValue[int(dest[r - 1 - (self.k - 1)])]
                pastVal[0] *= self.base

            sourcePastVal[0] -= self.maxShiftedSourceValue[int(source[r - self.delay - (self.sourceHistoryEmbedLength - 1) * self.sourceEmbeddingDelay])]
    ...
    def calcLocalTE(self):
        #if (torch.count_nonzero(self.xs_) == 0 and torch.count_nonzero(self.ys) == 0):
